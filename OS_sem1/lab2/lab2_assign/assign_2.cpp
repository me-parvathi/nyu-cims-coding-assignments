...
void print_summary(){
    ...
    double cpu_util = 0 ;
    double io_util = 0;
    double avg_turnarount_time = 0;
    double avg_wait_time =0 ;
    double throughput = 0;
    double wait_time_sum= 0, turnaround_time_sum =0;
    for (Process* proc : procQ){
    ...
    if(final_time!=0) cpu_util = cpu_counter/final_time;
    cpu_util = cpu_util * 100;
    if(final_time!=0) io_util = io_counter.time_counter / final_time ;
    io_util = io_util * 100;
    if(procQ.size()>0) avg_wait_time = wait_time_sum / procQ.size();
    if(procQ.size()>0) avg_turnarount_time = turnaround_time_sum / procQ.size();
    if(final_time!=0) throughput  = (procQ.size()/final_time) * 100;
