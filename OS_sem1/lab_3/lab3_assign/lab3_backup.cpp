...
class Process{
    ...
    public:
        ...
        unsigned int get_pte_frame_no(int vpage_num){
            return page_table[vpage_num].FRAME_NO;
        }

        void reset_pte(int vpage_num){
            page_table[vpage_num] = {0};
        }
...
class Clock_pager : public pager {
    ...
    public:
        ...
        frame_t* select_victim_frame(){
            ...
            if(myverbose) cout << "Referenced bit of vpage " << vpage<< " is   --> " << active_process->get_pte_referenced_bit(vpage) <<"\n";
            while(active_process->get_pte_referenced_bit(vpage)==1){
                active_process->unset_pte_refereneced(vpage);
                hand_at = (hand_at + 1) % num_frames;
            ...
            hand_at = (hand_at + 1) % num_frames;
             ...
             if(myverbose) cout << "Referenced bit is -->" << active_process->get_pte_referenced_bit(vpage) <<"\n";
            return victim_frame;
        }
    ...
    public:
        ...
        frame_t* select_victim_frame(){
            ...
            for(int i = 0; i< num_frames ; ++i){
                ...
                int curr_class = get_class(active_process->get_pte_referenced_bit(vpage_num), active_process->get_pte_modified_bit(vpage_num));
                ...
                if(reset){
                    active_process->unset_pte_refereneced(vpage_num);
                }
                hand_at = (hand_at + 1) % num_frames;
    ...
    public:
        ...
        frame_t* select_victim_frame(){
            ...
            for(int i = 0; i < num_frames ; ++i ){
                ...
                curr_frame->age >>= 1;
                if(active_process->get_pte_referenced_bit(curr_vpage_num)){
                    curr_frame->age |= 0x80000000;
                    active_process->unset_pte_refereneced(curr_vpage_num);
                }
    ...
    public:
        ...
        frame_t* select_victim_frame(){
            ...
            for(int i = 0; i< num_frames ; ++i ){
                ...
                if(instruction_counter > (curr_frame->age+ TAU) && active_process->get_pte_referenced_bit(curr_vpage_num)==0){
...
void do_read_write_operation(int vpage_num, char operation){
   ...
   //check the present bit of the pte
    ...
    if(frame_no !=-1){
        ...
        if(myverbose) printf("Setting pte_present of vpage %d to %d\n", vpage_num, active_process->is_pte_present(vpage_num));
...
void do_exit_operation(int pid){
    ...
    for(int vpage_num = 0 ; vpage_num < MAX_VPAGES; ++vpage_num){
        ...
        if(exiting_process->is_pte_present(vpage_num)){
            ...
            unsigned int frame_no = exiting_process->get_pte_frame_no(vpage_num);
            frame_table[frame_no].age = 0;
            frame_table[frame_no].frame_id = 128;
            frame_table[frame_no].process = -1;
            frame_table[frame_no].vpage = -1;
