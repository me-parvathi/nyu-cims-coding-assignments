...
struct
  ...
  fun bracket_depths json_chars in_string_flags =
    ...
    let
      ...
      (* for every bracket outside string it is either an opening or closing barackt
      1. For the opening one, it is either marked it as 1,0 or 0,1 
      2. For the closing ones it is either marked as -1,0 or0,-1 
     *)
      val mark_the_depth = Parallel.tabulate (0, N) (fn i =>
        if Seq.nth in_string_flags i then (0, 0)
        else
          let val c = Seq.nth json_chars i
          in
            if not (is_bracket c) then (0, 0)
            else case c of
              #"{" => (1, 0)
            | #"}" => (~1, 0)
            | #"[" => (0, 1)
      ...
      (* Add the depth markings calculated earlier, so
      when an opening or closing bracket is encountered, the running sum is updated.  *)
      val scan_result = Parallel.scan
        (fn ((a1, a2), (b1, b2)) => (a1 + b1, a2 + b2))
        (0, 0)
        (0, N)
        (fn i => Seq.nth mark_the_depth i)

      val bracket_indices = Parallel.filter
        (0, N)
        (fn i => i)
        (fn i => not (Seq.nth in_string_flags i) andalso is_bracket (Seq.nth json_chars i))

      val result = Seq.map
        (fn i =>
          let
            val c = Seq.nth json_chars i
            val (pre_dc, pre_ds) = Seq.nth scan_result i
            (* needs to be subtracted to get the right result *)
            val (dc, ds) = case c of
              #"}" => (pre_dc - 1, pre_ds)
            | #"]" => (pre_dc, pre_ds - 1)
            | _    => (pre_dc, pre_ds)
